...
async def send_emoji_interaction(event: NewMessage.Event, msgid, emoticon="❤️"):
    ...
    interaction_json = {"v": 1, "a": interactions}
...
async def send_emoji_reaction(event: NewMessage.Event, msgid, emoticon="❤️"):
    ...
    try:
        ...
        await telegram_log(
            f"Reaction {emoticon} sent successfully", topic_id=AUTO_REPLY_THREAD, level="DEBUG"
        )
    except Exception as e:  # pylint: disable=broad-except
        await telegram_log(
            f"Error sending emoji reaction: {e}", topic_id=AUTO_REPLY_THREAD, level="ERROR"
        )
...
async def process_reply(event: NewMessage.Event):
    ...
    """
    sent_message = await client.send_message(event.peer_id.user_id, message="❤️")
...
async def handle_message(event: NewMessage.Event):
    ...
    if event.is_private:
        ...
        if any(phrase in message_text.lower() for phrase in trigger_phrases):
            ...
            await telegram_log(
                f"Received test phrase: {message_text}", topic_id=AUTO_REPLY_THREAD, level="INFO"
            )
            user_link = f"[{user_id}](tg://openmessage?user_id={user_id})"
            await telegram_log(
                f"Triggering emoji for user ID {user_link} " f"with {repeat_count} repeats",
...
async def main():
    ...
    await telegram_log("Test script started", topic_id=AUTO_REPLY_THREAD, level="INFO")
